...
uint8_t ULT_Connection_Test() {
  ...
  HAL_Delay(100);
  HAL_Delay(3000);
  if (rxDATA_u1[0] == 0xFF || rxDATA_u1[1] == 0xFF || rxDATA_u1[2] == 0xFF ||
      rxDATA_u1[3] == 0xFF)
    state = 1;
  return state;
}

void ULT_getData() { HAL_Delay(2000); }
...
void ULT_Data_Read(void) {
  ...
  //	}
  //		user_main_printf("HEX:%02x %02x %02x %02x %02x %02x %02x %02x %02x
  //%02x %02x\r\n",
...
void ULT_Check_Sum(void) {
  ...
  for (uint8_t i = 0; i < 30; i++) {
    if (rxDATA_u1[n] == 0xFF) {
      if (rxDATA_u1[n + 3] ==
          ((rxDATA_u1[n] + rxDATA_u1[n + 1] + rxDATA_u1[n + 2]) & 0x00FF)) {
