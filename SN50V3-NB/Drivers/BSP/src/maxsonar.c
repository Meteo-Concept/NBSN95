...
void MAXSONAR_Init(void (*uart_init)(UART_HandleTypeDef *uartHandle),
                   void (*txs_init)(void)) {
  ULT_Rest();
  if (uart_init != NULL)
    uart_init(&huart1);
...
void MAXSONAR_DeInit(void (*uart_disable)(UART_HandleTypeDef *uartHandle),
                     void (*txs_deinit)(void)) {
...
void MAXSONAR_Check_Sum(void) {
  uint8_t i = 0;
  for (int a = 0; a < 200; a++) {
    if (receive_data_ult[a] == 'R' &&
        (receive_data_ult[a + 1] >= '0' && receive_data_ult[a + 1] <= '9')) {
