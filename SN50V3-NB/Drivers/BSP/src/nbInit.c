...
NB nb = {.net_flag = no_status,
         .recieve_flag = 0,
         .usart.len = 0,
         .usart.data = recieve_data,
         .imei = {0},
         .imsi = {0},
         .singal = 0};
 ...
 */
NB_TaskStatus nb_null_run(const char *param) { return NB_CMD_FAIL; }
...
NB_TaskStatus nb_cfunsta_get(const char *param) { return nb_cmd_status; }
...
NB_TaskStatus nb_cfunoff_get(const char *param) { return nb_cmd_status; }
...
NB_TaskStatus nb_cfun_get(const char *param) { return nb_cmd_status; }
...
NB_TaskStatus nb_qsclk_get(const char *param) { return nb_cmd_status; }
...
NB_TaskStatus nb_qicfg_get(const char *param) { return nb_cmd_status; }
...
NB_TaskStatus nb_qsclkoff_get(const char *param) { return nb_cmd_status; }
...
NB_TaskStatus nb_cgdcont_get(const char *param) { return nb_cmd_status; }
/**
...
NB_TaskStatus nb_cpsms_get(const char *param) { return nb_cmd_status; }
...
NB_TaskStatus nb_qdns_get(const char *param) {
  ...
  if (p1 == NULL || q1 == NULL) {
    ...
    sprintf(record_log + strlen(record_log),
...
ATCmdNum NBTASK(uint8_t *task) {
  ...
  case _AT_QDNS: {
    if ((is_ipv4_addr((char *)user.add) == 1) ||
        (is_ipv4_addr((char *)user.add) == 2) || (nds_timer_flag2 == 1) ||
        (is_ipv6_addr((char *)user.add) == 1) ||
        (is_ipv6_addr((char *)user.add) == 2)) {
      *task = _AT_UPLOAD_START;
  ...
  case _AT_COAP_CONFIG:
    if (strstr((char *)user.add, "NULL") != NULL ||
        strstr((char *)user.uri1, "NULL") != NULL) {
      *task = _AT_UPLOAD_END;
  ...
  case _AT_COAP_QCOAPHEAD:
    if (NBTask[_AT_COAP_QCOAPHEAD].run(NULL) == NB_CMD_SUCC) {
      *task = _AT_COAP_OPTION1;
      user_main_printf(
          "Set the CoAP message ID and automatically generate a token");
      sprintf(record_log + strlen(record_log),
              "Set the CoAP message ID and automatically generate a token\r\n");
    } else {
      at_state = _AT_ERROR;
  ...
  case _AT_MQTT_Config:
    if (strstr((char *)user.add, "NULL") != NULL ||
        strstr((char *)user.pubtopic, "NULL") != NULL) {
      *task = _AT_UPLOAD_END;
  ...
  case _AT_UDP_URI:
    ...
    if (uri_state == NB_STA_SUCC) {
      ...
      *task = _AT_UDP_SEND;
      ...
      sprintf(record_log + strlen(record_log),
              "Failed to upload data,resend data\r\n");
    } else {
      *task = _AT_IDLE;
    }
    break;
  ...
  case _AT_TCP_URI:
    ...
    //			{
    //				*task = _AT_TCP_SEND;
    //				user_main_printf("Open a Socket Service
    //successfully");
    //			}
...
void stored_datalog(void) {
  ...
  for (uint16_t i = 0, j = 0; i < strlen((char *)record_log); i = i + 4, j++)
    parameters_log[j] = record_log[i + 0] << 24 | record_log[i + 1] << 16 |
                        record_log[i + 2] << 8 | record_log[i + 3];
  FLASH_erase(
      FLASH_USER_START_DATALOG + sys.log_seq * FLASH_PAGE_SIZE * 4,
      ((FLASH_USER_START_DATALOG + (sys.log_seq + 1) * FLASH_PAGE_SIZE * 4) -
       (FLASH_USER_START_DATALOG + sys.log_seq * FLASH_PAGE_SIZE * 4)) /
          FLASH_PAGE_SIZE);
  FLASH_program(FLASH_USER_START_DATALOG + sys.log_seq * FLASH_PAGE_SIZE * 4,
                parameters_log, sizeof(parameters_log) / 4);
  sys.log_seq++;
  if (sys.log_seq == 20)
    sys.log_seq = 0;
  config_Set();
