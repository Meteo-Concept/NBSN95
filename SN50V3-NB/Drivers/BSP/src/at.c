...
ATEerror_t at_pword_set(const char *param) {
  ...
  password_parameters[0] =
      sys.pwd[0] << 24 | sys.pwd[1] << 16 | sys.pwd[2] << 8 | sys.pwd[3];
  password_parameters[1] =
      sys.pwd[4] << 24 | sys.pwd[5] << 16 | sys.pwd[6] << 8 | sys.pwd[7];

  FLASH_erase(FLASH_USER_START_PASSWORD,
              (FLASH_USER_END - FLASH_USER_START_PASSWORD) / FLASH_PAGE_SIZE);
  FLASH_program(FLASH_USER_START_PASSWORD, password_parameters,
                sizeof(password_parameters) / 4);
...
ATEerror_t at_deui_set(const char *param) {
  ...
  if (strlen(param) - (pos - param) - 1 != 15 ||
      hexDetection((char *)(param + (pos - param) + 1)) == 0) {
    return AT_PARAM_ERROR;
  }

  memset(user.deui, 0, sizeof(user.deui));
  memcpy(user.deui, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
...
ATEerror_t at_fdr_run(const char *param) {
  ...
  HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_WORD,
                                 EEPROM_USER_START_FDR_FLAG,
                                 0x01); // store fdr_flag
  HAL_FLASHEx_DATAEEPROM_Lock();
  FLASH_erase(FLASH_USER_START_PASSWORD,
              (FLASH_USER_END - FLASH_USER_START_PASSWORD) / FLASH_PAGE_SIZE);
  FLASH_erase(FLASH_USER_START_ADDR_CONFIG,
              (FLASH_USER_END_ADDR - FLASH_USER_START_ADDR_CONFIG) /
                  FLASH_PAGE_SIZE);
  FLASH_erase(FLASH_USER_COAP_URI1,
              (FLASH_USER_COAP_END - FLASH_USER_COAP_URI1) / FLASH_PAGE_SIZE);
  DatalogClear();
  ...
  FLASH_program(FLASH_USER_START_ADDR_CONFIG, general_parameters,
                sizeof(general_parameters) / 4);
  NVIC_SystemReset();
  return AT_OK;
}
...
ATEerror_t at_servaddr_get(const char *param) {
  ...
  if (strlen((char *)user.add_ip) != 0 &&
      strstr((char *)user.add_ip, "NULL") == NULL)
...
ATEerror_t at_servaddr_set(const char *param) {
  ...
  memcpy(user.add, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
...
ATEerror_t at_client_set(const char *param) {
  ...
  memcpy(user.client, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
...
ATEerror_t at_uname_set(const char *param) {
  ...
  memcpy(user.uname, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
...
ATEerror_t at_pwd_set(const char *param) {
  ...
  memcpy(user.pwd, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
  return AT_OK;
}
/************** 			AT+PUBTOPIC		 **************/
...
ATEerror_t at_pubtopic_set(const char *param) {
  ...
  memcpy(user.pubtopic, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
  return AT_OK;
}
/************** 			AT+SUBTOPIC		 **************/
...
ATEerror_t at_subtopic_set(const char *param) {
  ...
  memcpy(user.subtopic, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
  return AT_OK;
}
/************** 			AT+TDC		 **************/
...
ATEerror_t at_inmod_set(const char *param) {
  ...
  if (sys.inmod != '0' && sys.inmod != '1' && sys.inmod != '2' &&
      sys.inmod != '3') {
...
ATEerror_t at_dnscfg_set(const char *param) {
  ...
  memcpy(user.dns_add, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
  return AT_OK;
}
/************** 			AT+APN		**************/
ATEerror_t at_apn_set(const char *param) {
  ...
  memcpy(user.apn, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
  return AT_OK;
}
...
ATEerror_t at_clocklog_get(const char *param) {
  if (keep)
    printf("AT+CLOCKLOG=");
  printf("%d,%d,%d,%d\r\n", sys.clock_switch, sys.strat_time, sys.tr_time,
         sys.sht_noud);
  return AT_OK;
}
...
ATEerror_t at_uri1_set(const char *param) {
  ...
  memcpy(user.uri1, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
...
ATEerror_t at_uri2_set(const char *param) {
  ...
  memcpy(user.uri2, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
...
ATEerror_t at_uri3_set(const char *param) {
  ...
  memcpy(user.uri3, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
...
ATEerror_t at_uri4_set(const char *param) {
  ...
  memcpy(user.uri4, (param + (pos - param) + 1),
         strlen((param + (pos - param) + 1)));
...
void config_Set(void) {
  ...
  //	general_parameters[0]=sys.pwd[0]<<24 | sys.pwd[1]<<16 	| sys.pwd[2]<<8
  //| sys.pwd[3];
  //	general_parameters[1]=sys.pwd[4]<<24 | sys.pwd[5]<<16 	| sys.pwd[6]<<8
  //| sys.pwd[7];
  ...
  general_parameters[11] =
      qband_flag << 24 | sys.tr_time << 16 | noud_flags << 8 | sys.sht_noud;
  general_parameters[12] =
      sys.platform << 24 | sys.dns_timer << 16 | sys.dns_time << 8;
  general_parameters[28] =
      mqtt_qos_flags << 24 | mqtt_qos << 16 | sys.cert << 8 | sys.tlsmod;
  general_parameters[29] =
      sys.clock_switch << 24 | sys.strat_time << 8 | sys.log_seq;

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.deui); i = i + 4, j++)
    general_parameters[7 + j] = user.deui[i + 0] << 24 |
                                user.deui[i + 1] << 16 | user.deui[i + 2] << 8 |
                                user.deui[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.apn); i = i + 4, j++)
    general_parameters[18 + j] = user.apn[i + 0] << 24 | user.apn[i + 1] << 16 |
                                 user.apn[i + 2] << 8 | user.apn[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.dns_add); i = i + 4, j++)
    general_parameters[13 + j] = user.dns_add[i + 0] << 24 |
                                 user.dns_add[i + 1] << 16 |
                                 user.dns_add[i + 2] << 8 | user.dns_add[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.add); i = i + 4, j++)
    servaddr_parameters[0 + j] = user.add[i + 0] << 24 | user.add[i + 1] << 16 |
                                 user.add[i + 2] << 8 | user.add[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.uname); i = i + 4, j++)
    mqtt_parameters_uname[j] = user.uname[i + 0] << 24 |
                               user.uname[i + 1] << 16 |
                               user.uname[i + 2] << 8 | user.uname[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.pwd); i = i + 4, j++)
    mqtt_parameters_pwd[j] = user.pwd[i + 0] << 24 | user.pwd[i + 1] << 16 |
                             user.pwd[i + 2] << 8 | user.pwd[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.client); i = i + 4, j++)
    mqtt_parameters_client[j] = user.client[i + 0] << 24 |
                                user.client[i + 1] << 16 |
                                user.client[i + 2] << 8 | user.client[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.pubtopic); i = i + 4, j++)
    mqtt_parameters_pubtopic[j] =
        user.pubtopic[i + 0] << 24 | user.pubtopic[i + 1] << 16 |
        user.pubtopic[i + 2] << 8 | user.pubtopic[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.subtopic); i = i + 4, j++)
    mqtt_parameters_subtopic[j] =
        user.subtopic[i + 0] << 24 | user.subtopic[i + 1] << 16 |
        user.subtopic[i + 2] << 8 | user.subtopic[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.uri1); i = i + 4, j++)
    coap_parameters1[j] = user.uri1[i + 0] << 24 | user.uri1[i + 1] << 16 |
                          user.uri1[i + 2] << 8 | user.uri1[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.uri2); i = i + 4, j++)
    coap_parameters2[j] = user.uri2[i + 0] << 24 | user.uri2[i + 1] << 16 |
                          user.uri2[i + 2] << 8 | user.uri2[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.uri3); i = i + 4, j++)
    coap_parameters3[j] = user.uri3[i + 0] << 24 | user.uri3[i + 1] << 16 |
                          user.uri3[i + 2] << 8 | user.uri3[i + 3];

  for (uint8_t i = 0, j = 0; i < strlen((char *)user.uri4); i = i + 4, j++)
    coap_parameters4[j] = user.uri4[i + 0] << 24 | user.uri4[i + 1] << 16 |
                          user.uri4[i + 2] << 8 | user.uri4[i + 3];
  ...
  FLASH_program(FLASH_USER_START_ADDR_CONFIG, general_parameters,
                sizeof(general_parameters) / 4);
  FLASH_program(FLASH_USER_START_SERVADDR_ADD, servaddr_parameters,
                sizeof(servaddr_parameters) / 4);
  //	FLASH_program(FLASH_USER_START_COAP,coap_parameters,
  //sizeof(coap_parameters)/4);
  FLASH_program(FLASH_USER_START_MQTT_CLIENT, mqtt_parameters_client,
                sizeof(mqtt_parameters_client) / 4);
  FLASH_program(FLASH_USER_START_MQTT_UNAME, mqtt_parameters_uname,
                sizeof(mqtt_parameters_uname) / 4);
  FLASH_program(FLASH_USER_START_MQTT_PWD, mqtt_parameters_pwd,
                sizeof(mqtt_parameters_pwd) / 4);
  FLASH_program(FLASH_USER_START_MQTT_PUBTOPIC, mqtt_parameters_pubtopic,
                sizeof(mqtt_parameters_pubtopic) / 4);
  FLASH_program(FLASH_USER_START_MQTT_SUBTOPIC, mqtt_parameters_subtopic,
                sizeof(mqtt_parameters_subtopic) / 4);
  FLASH_program(FLASH_USER_COAP_URI1, coap_parameters1,
                sizeof(coap_parameters1) / 4);
  FLASH_program(FLASH_USER_COAP_URI2, coap_parameters2,
                sizeof(coap_parameters2) / 4);
  FLASH_program(FLASH_USER_COAP_URI3, coap_parameters3,
                sizeof(coap_parameters3) / 4);
  FLASH_program(FLASH_USER_COAP_URI4, coap_parameters4,
                sizeof(coap_parameters4) / 4);
}
...
void config_Get(void) {
  ...
  if (sys.protocol != COAP_PRO && sys.protocol != UDP_PRO &&
      sys.protocol != MQTT_PRO && sys.protocol != TCP_PRO)
