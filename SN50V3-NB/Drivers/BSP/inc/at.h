...
struct ATCommand_s {
  ...
  ATEerror_t (*get)(
      const char *param); /*< =? after the string to get the current value*/
  ATEerror_t (*set)(const char *param); /*< = (but not =?\0) after the string to
                                           set a value */
  ATEerror_t (*run)(
      const char *param); /*< \0 after the string - run the command */
#if !defined(NO_HELP)
...
#ifndef NO_HELP
        .help_string = AT FDR1 "     : Reset parameters to factory default "
                               "values except for passwords",
#endif
        .get = at_return_error,
        .set = at_return_error,
        .run = at_fdr1_run,
...
#ifndef NO_HELP
        .help_string = AT TDC
        "     : Get or set the application data transmission interval in s",
#endif
        .get = at_tdc_get,
        .set = at_tdc_set,
        .run = at_return_error,
...
#ifndef NO_HELP
        .help_string =
            AT INTMOD "  : Get or Set the trigger interrupt mode "
                      "(0:input,1:falling or rising,2:falling,3:rising)",
#endif
        .get = at_inmod_get,
        .set = at_inmod_set,
        .run = at_return_error,
...
#ifndef NO_HELP
        .help_string = AT PRO
        "     : Get or Set usage agreement (1:COAP,2:UDP,3:MQTT,4:TCP)",
#endif
        .get = at_pro_get,
        .set = at_pro_set,
        .run = at_return_error,
