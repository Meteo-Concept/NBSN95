...
NBState sockCreat(void) {
  ...
  //				if(errorNum > 5)
  //				{
  //					user_main_printf("Failed to create
  //socket."); 					nb_Reboot(); 					return fail;
  //				}
  //		}
  //		else
  //		{
  ...
  //		sprintf(singalBuff+strlen(singalBuff), "%c",
  //nb.recieve_data[7+i]);
  //
...
NBState sockClosd(void) { return success; }

NBState dataSendUDP(char *payload) { return success; }
