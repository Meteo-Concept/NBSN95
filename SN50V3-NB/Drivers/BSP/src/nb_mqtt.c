 ...
 * @retval None
 */
NB_TaskStatus nb_MQTT_data_read_run(const char *param) { return nb_cmd_status; }

NB_TaskStatus nb_MQTT_data_read_set(const char *param) {
  memset(downlink_data, 0, 1000);
  char *pos_start = strstr((char *)nb.usart.data, "\",\"");
  user_main_debug("pos_start:%p", &pos_start);
  if (pos_start[3] == '{') {
    char *pos_end = strchr(pos_start, '}');
    user_main_debug("pos_end:%p", &pos_end);
    memcpy(downlink_data,
           &nb.usart.data[pos_start - ((char *)nb.usart.data) + 3],
           (pos_end - pos_start - 2));
    at_downlink_flag = 1;
  } else {
    char *pos_end = strchr(pos_start, '\n');
